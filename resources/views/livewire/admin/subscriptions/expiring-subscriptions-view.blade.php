<x-ui.dashboard.page-wrapper>
    <x-ui.dashboard.page-header
        ...
        :subtitle="__('Filter expiring subscriptions by member, plan, or expiry window, then queue reminders for the visible set.')"
    >
        <x-slot name="actions">
            <flux:button
                wire:click="loadExpiringSubscriptions"
                wire:loading.attr="disabled"
                wire:target="loadExpiringSubscriptions"
                icon="arrow-path"
            >
            ...
            <flux:button
                ...
                wire:loading.attr="disabled"
                wire:target="sendReminderToAll"
                icon="bell-alert"
            >
                <span wire:loading.remove wire:target="sendReminderToAll">{{ __('Send Reminder to Filtered') }}</span>
                    ...
                    <tr wire:key="expiring-subscription-{{ $subscription->id }}" class="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/70">
                        ...
                        <td class="px-4 py-4">
                            <x-ui.dashboard.row-actions justify="start">
                                <flux:button
                                    wire:click="sendReminder({{ $subscription->id }})"
                                    wire:loading.attr="disabled"
                                    wire:target="sendReminder({{ $subscription->id }})"
                                    size="sm"
                                >
