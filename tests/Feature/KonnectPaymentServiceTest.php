...
test('konnect payment service verifies and refunds payments through the sandbox network endpoint', function () {
    Http::fake([
        'https://api.sandbox.konnect.network/api/v2/payments/TXN-VERIFIED' => Http::response([
            'status' => 'completed',
            'amount' => 12500,
            'paymentRef' => 'TXN-VERIFIED',
            'createdAt' => now()->toIso8601String(),
        ], 200),
        'https://api.sandbox.konnect.network/api/v2/payments/TXN-VERIFIED/refund' => Http::response([
            'refundRef' => 'REFUND-123',
        ], 200),
    ]);
    ...
    $service = app(KonnectPaymentService::class);

    $verified = $service->verifyPayment('TXN-VERIFIED');
    $refunded = $service->refund('TXN-VERIFIED', 12.500);

    expect($verified['success'])->toBeTrue();
    expect($verified['status'])->toBe('completed');
    expect($verified['amount'])->toBe(12.5);
    expect($refunded['success'])->toBeTrue();
    expect($refunded['refund_id'])->toBe('REFUND-123');

    Http::assertSent(function ($request): bool {
        return in_array($request->url(), [
            'https://api.sandbox.konnect.network/api/v2/payments/TXN-VERIFIED',
            'https://api.sandbox.konnect.network/api/v2/payments/TXN-VERIFIED/refund',
        ], true);
    });
